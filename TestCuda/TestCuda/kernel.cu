...
Error:
	...
	*/
	...
	int* a = new int[_SIZE_];
	int* b = new int[_SIZE_];
	int* c = new int[_SIZE_];
	int* result = new int[_SIZE_];
	...
	clock_t startTimeCPU = clock();

	for (int i = 0; i < _SIZE_; i++)
	{
		c[i] = abs(powf(a[i], 2) - powf(b[i], 2));
	}


	clock_t stopTimeCPU = clock();

	std::cout << "Fini(GPU) en : " << stopTimeCPU - startTimeCPU << " ms" << std::endl;
	...
	*/
	...
	addLoopGPU << <64, 8 >> > (dev_a, dev_b, dev_result);
